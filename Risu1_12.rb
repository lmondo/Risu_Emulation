...
class Field                            #Field class    木の生えたフィールド、木の実を埋められたフィールドの２種フィールドがある。
    ...
    def fallin      #@trees = [#0[#0[x1, y1],　#1[UDRL]], [[x2, y1],　[UDRL]], ...]
        for i in 0...@n
            udrl = @trees[i][1]
            udrl.map{|v|
                x = v[0]
    ...
    def seed(x, y)          #2/3の確率で Risu の現在位置に木の実を埋める。
        if (rand(3) > 0) and (@seedfld[x][y] != "T")      #埋める場所に木が生えてないこと
            @seedfld[x][y] += 1
            self.cdisp
            @c += 1
        end
    ...
    def dig(x, y)       #1/2の確率で木の実を掘り起こす。
        if (rand(2) > 0) and (@seedfld[x][y] != "T")      #掘り起こす場所が木でないこと
            if @seedfld[x][y] >= 1
                @seedfld[x][y] -= 1
                puts "dig!"
                self.cdisp
